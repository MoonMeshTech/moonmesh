...
namespace IpFactor
{
	bool kOptionUsed = false;
	bool KM_LOCALHOST_USED = false;
	bool PUBLIC_IP_USED_FLAG = false;
...
int Config::_Parse(nlohmann::json json)
{
        ...
        {   
            for(auto& reconnoitre : sentinelNode){
                ...
                if(_ip.empty()){
                    _ip=execute_based_on_global_options();
                }
            }
        }
        else
...
std::string Config::execute_based_on_global_options(){
    std::string host = IpFactor::kPublicIpAddress;
    std::string path = "/GetPublicIp";
    int port = _httpPort;
    if (IpFactor::PUBLIC_IP_USED_FLAG) 
    {          
    ...
    else if (IpFactor::KM_LOCALHOST_USED) 
    {
        
        return "";
    }
    else if(IpFactor::kOptionUsed)
    {
...
void update_config_ip(const std::string& newIP) {
    ...
    if (inFile.good()) {
        configJson = json::parse(inFile);
        inFile.close();
    } else {
        configJson = json::parse(global::ca::kConfigJson);
    }

    configJson["ip"] = newIP;
    configJson["http_callback"]["ip"] = newIP;

    std::ofstream outFile("config.json");
    outFile << configJson.dump(4);
    outFile.close();
    std::cout << "IP updated to: " << newIP << std::endl;
}

std::vector<std::string> Config::_ReadTrackerIPs()
{
    if(global::GetBuildType() == GenesisConfig::BuildType::BUILD_TYPE_PRIMARY)
    {
        return {"52.8.162.216", "50.18.55.167"};
    }
    else if (global::GetBuildType() == GenesisConfig::BuildType::K_BUILD_TYPE_TEST)
    {
        return {"52.8.162.216", "50.18.55.167"};
    }
    else if (global::GetBuildType() == GenesisConfig::BuildType::BUILD_TYPE_DEV)
    {
        return {};
    }
}