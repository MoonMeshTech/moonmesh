...
#ifndef EVM_HOST_HEADER
#define EVM_HOST_HEADER
...
{
public:
    explicit EvmHost(contractDataContainer *contractDataStorage = nullptr);
    ...
    mutable std::unordered_map<evmc::address, StateAccount> accounts;
    ...
    static constexpr auto maxRecordedAccountAccesses = 200;
    ...
    std::vector<selfDestructRecord> recorded_selfdestructs;

    // The copy of call inputs for the recorded_calls record.
    std::vector<evmc::bytes> callInputsRecorded;
    ...
    void recordAccountAccess(const evmc::address &addr) const;
    ...
    std::vector<TransferInfo> coinTransfersInProgress;
