...
#ifndef EVM_HOST_DATA_HEADER
#define EVM_HOST_DATA_HEADER
...
struct StateAccount
{
    ...
    void CreateTrie(const std::string &rootHash, const std::string &ContractAddr, contractDataContainer *contractDataStorage)
    {
        if (rootHash.empty())
        {
            storageRoot = std::make_shared<Trie>(ContractAddr, contractDataStorage);
        }
        else
        {
            storageRoot = std::make_shared<Trie>(rootHash, ContractAddr, contractDataStorage);
        }
    ...
    void set_code(const evmc::bytes &code_) noexcept
    {
        ...
        std::string hash = Keccak256CrypterOpenSSL(input);
...
struct selfDestructRecord
{
    ...
    // Equal operator.
    bool operator==(const selfDestructRecord &other) const noexcept
    {
