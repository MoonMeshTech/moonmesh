        ...
        for (auto &iter : coinTransfersInProgress)
        {
            ...
            if (iter.to == fromAddr)
            {
                ...
                else
                {
                    ...
                    iter.amount = 0;
                    uint64_t balance = 0;
                    get_balance_by_utxo(fromAddr, global::ca::ASSET_TYPE_VOTE, balance);
...
evmc::uint256be EvmHost::get_balance(const evmc::address &addr) const noexcept
{
    ...
    if (get_balance_by_utxo(Addr, global::ca::ASSET_TYPE_VOTE, amount) != 0)
    {
        amount = 0;
    }
...
std::string EvmHost::calculateCreate2Address(const evmc_message& message) noexcept
{
    ...
    std::memcpy(creationSeed + offset, initCodeHash, kKeccakHashSize);

    uint8_t* addressBase = ethash::keccak256(creationSeed,creationSeedSize).bytes;
    static constexpr size_t evmAddrOffset = kKeccakHashSize - EVM_ADDRESS_SIZE;
    evmc_address address = {0};
    std::memcpy(address.bytes, addressBase + evmAddrOffset, sizeof address);
