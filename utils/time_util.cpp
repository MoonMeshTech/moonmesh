...
x_uint64_t TimeUtil::getTimestampPerUnit(const x_uint64_t& utcTime)
{
...
x_uint64_t TimeUtil::getNtpTimestampConfig()
{
...
void TimeUtil::testNtpDelay()
{
    ...
    {
        timeValue_ = 0ULL;
        err = ntpGetTimeTest(itvec->c_str(), NTP_PORT, 1000, &timeValue_);
        if (0 != err)
        {
...
{
    return ((TxTime/1000000) - (global::ca::GetGenesisTime()/1000000)) / (24  *60 * 60);
}
