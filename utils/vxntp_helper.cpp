 ...
 * @param [out] timeValue_ : The answer time value returned by the successful operation in 100 nanoseconds from January 1, 1970 to the present.
 * 
 * @return x_int32_t
 * Success, returning 0;
 * - Failed, returned error code.
 */
x_int32_t ntp_get_time(x_cstring_t xszt_host, x_uint16_t xut_port, x_uint32_t xut_tmout, x_uint64_t * timeValue_)
{
    x_int32_t xit_err = -1;
    ...
    if ((X_NULL == xszt_host) || (xut_tmout <= 0) || (X_NULL == timeValue_))
    {
        return -1;
    }
    ...
    for (std::vector< std::string >::iterator itvec = xvec_host.begin(); itvec != xvec_host.end(); ++itvec)
    {
        ...
        if (0 == xit_err)
        {
            ...
            *timeValue_ = xit_tmlst[3] + ((xit_tmlst[1] - xit_tmlst[0]) + (xit_tmlst[2] - xit_tmlst[3])) / 2;
            break;
        }
    }
    return xit_err;
...
x_int32_t ntpGetTimeTest(x_cstring_t xszt_host, x_uint16_t xut_port, x_uint32_t xut_tmout, x_uint64_t * timeValue_)
{
    x_int32_t xit_err = -1;
    std::vector< std::string > xvec_host;
