...
{       
    ...
    std::string hex_vector =  Str2Hex(ivec);

    key =   keyHex + "_" + hex_vector;
...
int Envelop::EnvelopeWrapper(const std::string &data, const std::string& pubstr, std::string& strEncTxt, std::string& cipherText, std::string& out)
{
   EVP_PKEY*pkeyDer = NULL;
   get_ed_pub_key_by_bytes(pubstr, pkeyDer);
...
int Envelop::OpenEnvelope(const std::string& cipherText, const std::string& out, const std::string& strEncTxt, const std::string& pub_str, std::string& message)
{
    ...
    Account account;
	if(MagicSingleton<AccountManager>::GetInstance()->getAccountPublicKeyByBytes(pub_str, account) == false){
		return -3;
	}
...
void TestFunction2()
{
    ...
    for(int i = 0; i < 1; ++i)
    {
        ...
        if(enve.EnvelopeWrapper(data_1,pubstr,strEncTxt,cipherText,sign_message) != 0)
        {
            errorL("EnvelopeWrapper fail !");
            return;
        }
