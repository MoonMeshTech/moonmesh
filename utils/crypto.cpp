...
bool crypto::generate_ecdh_keys(uint8_t ecdh_public_key[EC_PUBLIC_KEY_LENGTH],
                              uint8_t ecdh_private_key[EC_PRIVATE_KEY_LENGTH])
{
    int len = 0;
    bool ret = false;
    ...
    if (EC_METHOD_get_field_type(EC_GROUP_method_of(group)) == NID_X9_62_prime_field)
    {
        ...
        len = EC_POINT_point2oct(group,
                                 ...
                                 ecdh_public_key,
                                 EC_PUBLIC_KEY_LENGTH, NULL);
        if (len != EC_PUBLIC_KEY_LENGTH)
        {
...
bool crypto::calculate_ecdh_shared_key(const uint8_t ecdh1_public_key[EC_PUBLIC_KEY_LENGTH],
                                 const uint8_t ecdh1_private_key[EC_PRIVATE_KEY_LENGTH],
                                 const uint8_t ecdh2_public_key[EC_PUBLIC_KEY_LENGTH],
                                 uint8_t ecdh_shared_key[ECDH_SHARED_KEY_LENGTH])
{
    int len = 0;
    int ret = false;
    EC_KEY *ecdh = EC_KEY_new();
    ...
    if (EC_METHOD_get_field_type(EC_GROUP_method_of(group)) == NID_X9_62_prime_field)
    {
        ...
        len = ECDH_compute_key(ecdh_shared_key,
                               ECDH_SHARED_KEY_LENGTH,
                               p_ecdh2_public,
                               ecdh,
                               NULL);
        if (len != ECDH_SHARED_KEY_LENGTH)
        {
...
bool crypto::ecdsa_sign(const uint8_t ec_private_key[EC_PRIVATE_KEY_LENGTH], uint8_t *hash, uint8_t hash_len, uint8_t sign[CRYPTO_ECDSA_SIG_LEN])
{
    int len = 0;
    int ret = false;
...
bool crypto::ecdsa_verify(const uint8_t ec_public_key[EC_PUBLIC_KEY_LENGTH], const uint8_t *hash, int hash_len, const uint8_t sign[CRYPTO_ECDSA_SIG_LEN])
{
    int len = 0;
    int ret = false;
    EC_KEY *eckey = EC_KEY_new();
...
bool crypto::createHkdfBytes(const uint8_t ecdh_shared_key[ECDH_SHARED_KEY_LENGTH],
                               ...
                               uint8_t out[])
{
    ...
    if (EVP_PKEY_CTX_set1_hkdf_key(pctx, ecdh_shared_key, ECDH_SHARED_KEY_LENGTH) <= 0)
        goto err;
...
err:
    ...
    unsigned char prk[ECDH_SHARED_KEY_LENGTH], T[ECDH_SHARED_KEY_LENGTH]={0}, tmp[CRYPTO_AES_KEY_LEN];
    uint32_t outlen = ECDH_SHARED_KEY_LENGTH;
    int i, ret, tmplen;
    unsigned char *p;
    /*extract is a simple HMAC...
      Note:salt should be treated as hmac key and ikm should be treated as data
     */
    if (!HMAC(md, salt, CRYPTO_SALT_LEN, ecdh_shared_key, ECDH_SHARED_KEY_LENGTH, prk, &outlen))
        return false;
    ...
    */
    ...
    ret = CRYPTO_AES_KEY_LEN/ECDH_SHARED_KEY_LENGTH + !!(CRYPTO_AES_KEY_LEN%ECDH_SHARED_KEY_LENGTH);
    ...
    for (i = 0; i < ret; i++)
    {
        ...
        if (i != 0)
        {
            memcpy(p, T, ECDH_SHARED_KEY_LENGTH);
            p += ECDH_SHARED_KEY_LENGTH;
        }
        ...
        *p++ = i + 1;
        ...
        HMAC(md, prk, ECDH_SHARED_KEY_LENGTH, tmp, (int)(p - tmp), T, &outlen);
        memcpy(out + i*ECDH_SHARED_KEY_LENGTH, T, tmplen < ECDH_SHARED_KEY_LENGTH ? tmplen:ECDH_SHARED_KEY_LENGTH);
        tmplen -= ECDH_SHARED_KEY_LENGTH;
    }
